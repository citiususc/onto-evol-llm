...
# Original query to find deleted classes in Ontology 2
print("\nDeleted classes in Ontology 2:")
query = """
...
WHERE { 
  GRAPH <http://example.org/ontology1> { 
    ?deletedClass rdf:type owl:Class . 
  } 
   
  # Exclude classes that still exist in version 2 
  FILTER NOT EXISTS { 
...
for row in dataset.query(query):
    print(f"Deleted class: {row.deletedClass}")
