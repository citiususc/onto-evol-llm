...
SELECT DISTINCT ?newclass
WHERE {
  # Load version 1 of the ontology
  GRAPH <http://example.org/ontology1> {
    ?newclass rdf:type owl:Class .
  }

  # Load version 2 of the ontology
  GRAPH <http://example.org/ontology2> {
    ?newclass rdf:type owl:Class .
  }

  # Filter for only those classes that are present in version 1 but not in version 2
  FILTER EXISTS {
    GRAPH <http://example.org/ontology1> {
      ?newclass rdf:type owl:Class .
    }
